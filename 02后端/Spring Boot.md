...
==将主配置类（@SpringBootApplication标注的类）的**所在包及下面所有子包里面的所有组件扫描到Spring容器**；==
...
我们在主配置文件编写的时候，文件名可以是   application-{profile}.properties/yml，如创建了两个其它的配置文件，一个为application-dev.properties，另一个为application-prod.properties。我们可以分别使用不同的配置环境。

但是系统默认使用application.properties的配置；



### 2、yml支持多文档块方式

下面的文档块是用“---”分隔开的。它激活的就是prod文档块配置。
...
​	方法1：在主配置文件application.properties中添加语句  spring.profiles.active=dev

​	方法2：在target文件目录下cmd窗口执行以下语句，其中的.jar名是程序生成的jar包。

​		java -jar spring-boot-02-config-0.0.1-SNAPSHOT.jar --spring.profiles.active=dev；

​	方法3：在运行图标左边的Edit Configurations中修改配置Progarm arguments，在里面填入--spring.profiles.active=dev也能实现相同效果。

​	方法4：虚拟机参数，在运行图标左边的Edit Configurations中修改配置VM options，在里面填入

-Dspring.profiles.active=dev，效果一样。
...
SpringBoot会从这四个位置全部加载主配置文件；**互补配置**：高优先级没有配置的项会使用低优先级的配置；



==我们还可以通过spring.config.location来改变默认的配置文件位置==

**项目打包好以后，我们可以使用命令行参数的形式，启动项目的时候来指定配置文件的新位置；指定配置文件和默认加载的这些配置文件共同起作用形成互补配置；下面的application.properties是复制到G盘目录下的。**

使用命令行：在IDEA下端选择Terminal便捷控制台，可以直接访问当前项目，输入cd target，可以看到生成的jar包，然后输入以下命令：
...
==**由jar包外向jar包内进行寻找；**==直接将配置文件与jar包放在同一个文件位置
